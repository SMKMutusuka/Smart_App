...
// File: javascript.js
// Versi: v2026-09-24-final-bulk
// ⭐ UPDATE: Approval pakai TABEL + CHECKBOX + BULK APPROVE (1 call)
// =============================================
...
// =============================================
// APPROVAL DASHBOARD — ⭐ OPTIMIZED (1 CALL)
// =============================================
function loadApprovalDashboard() {
  ...
  showLoading();
  var filterKelas = document.getElementById('approval_filter_kelas')
    ? document.getElementById('approval_filter_kelas').value : '';
  ...
  google.script.run
    .withSuccessHandler(function(data) {
      ...
      data = data || {};
      var kelasList = Array.isArray(data.kelasList) ? data.kelasList : [];
      var stats = data.stats || { pending: 0, approved: 0, rejected: 0 };
      var pendingList = Array.isArray(data.pending) ? data.pending : [];
    ...
    .getApprovalDashboardData(filterKelas);
}

// =============================================
// RENDER APPROVAL LIST — ⭐ TABEL + CHECKBOX + BULK
// =============================================
function renderApprovalList(pendingList) {
  var container = document.getElementById('approval-list-container');
  if (!container) return;
  ...
  if (!pendingList || pendingList.length === 0) {
    container.innerHTML =
      '<div class="card" style="text-align:center;padding:32px;">' +
        ...
        '<p style="font-size:12.5px;color:var(--text-muted);margin-top:4px;">Semua absensi sudah diverifikasi.</p>' +
      '</div>';
    return;
  }
    ...
    '<div class="card" style="padding:12px 16px;background:#f8fafc;border:1px solid var(--border);margin-bottom:12px;">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">' +
        '<div style="font-size:13px;font-weight:700;color:#0f172a;">' +
          ...
          '<span id="approval-selected-count">0</span> dari ' + pendingList.length + ' dipilih' +
        '</div>' +
  ...
  var tableHtml =
    '<div class="laporan-container">' +
      '<div class="laporan-scroll">' +
        '<table class="table-laporan" style="min-width:900px;">' +
          '<thead>' +
            '<tr>' +
              '<th style="width:40px;">' +
                '<input type="checkbox" id="cb-header-approval" onchange="toggleSelectAllApproval(this.checked)" style="width:18px;height:18px;cursor:pointer;accent-color:var(--primary);">' +
              '</th>' +
  ...
  pendingList.forEach(function(item) {
    var statusBadge = item.Is_Auto_Alpa
      ? '<span style="background:#7c3aed;color:#fff;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;white-space:nowrap;">🤖 AUTO-ALPA</span>'
      : '<span class="badge-status-table badge-' + (item.Status || 'hadir').toLowerCase() + '" style="font-size:10px;padding:3px 10px;">' + (item.Status || '-') + '</span>';

    var buktiParts = [];
    if (item.Has_GPS) {
      buktiParts.push('<span style="font-size:10px;color:#0369a1;background:#e0f2fe;padding:2px 6px;border-radius:6px;font-weight:700;white-space:nowrap;">📍 ' + (item.Jarak_Meter || '?') + 'm</span>');
    }
    if (item.Has_Selfie) {
      buktiParts.push('<a href="' + escapeHtml(item.Selfie_Url) + '" target="_blank" style="font-size:10px;color:#065f46;background:#d1fae5;padding:2px 6px;border-radius:6px;font-weight:700;text-decoration:none;white-space:nowrap;">📷 Selfie</a>');
    }
    if (item.Link_Surat) {
      buktiParts.push('<a href="' + escapeHtml(item.Link_Surat) + '" target="_blank" style="font-size:10px;color:#1e40af;background:#dbeafe;padding:2px 6px;border-radius:6px;font-weight:700;text-decoration:none;white-space:nowrap;">📄 Surat</a>');
    }
    if (buktiParts.length === 0) buktiParts.push('<span style="font-size:10px;color:#94a3b8;">-</span>');

    tableHtml +=
      '<tr>' +
        '<td style="text-align:center;">' +
          '<input type="checkbox" class="cb-approval" value="' + escapeHtml(item.ID_Absen) + '" onchange="onApprovalCheckChange()" style="width:18px;height:18px;cursor:pointer;accent-color:var(--primary);">' +
        '</td>' +
        '<td style="text-align:center;font-size:11px;">' + escapeHtml(item.Tanggal) + '</td>' +
        '<td style="text-align:center;font-size:11px;"><strong>' + escapeHtml(item.Nama_Kelas || '-') + '</strong></td>' +
        '<td style="text-align:center;font-size:11px;font-family:monospace;">' + escapeHtml(item.NIS) + '</td>' +
        '<td style="text-align:left;font-size:12px;"><strong>' + escapeHtml(item.Nama_Siswa) + '</strong></td>' +
        '<td style="text-align:center;">' + statusBadge + '</td>' +
        '<td style="text-align:left;font-size:10.5px;color:#475569;max-width:180px;overflow:hidden;text-overflow:ellipsis;">' + (escapeHtml(item.Keterangan || '-').substring(0, 60)) + '</td>' +
        '<td style="text-align:center;display:flex;gap:4px;flex-wrap:wrap;justify-content:center;">' + buktiParts.join(' ') + '</td>' +
      '</tr>';
  });
...
// ⭐ Toggle pilih semua
function toggleSelectAllApproval(forceState) {
  var checkboxes = document.querySelectorAll('.cb-approval');
  ...
  checkboxes.forEach(function(cb) { cb.checked = targetState; });
  if (headerCb) headerCb.checked = targetState;

  onApprovalCheckChange();
}

// ⭐ Update UI saat checkbox berubah
function onApprovalCheckChange() {
  ...
  var counterEl = document.getElementById('approval-selected-count');
...
function getSelectedApprovalIds() {
  var checkboxes = document.querySelectorAll('.cb-approval:checked');
  var ids = [];
