...
function loadMonitoringPKLGuru() {
  ...
  container.innerHTML = '<div style="text-align:center;padding:32px;">' +
    ...
    '<p style="margin-top:12px;color:var(--text-muted);">Memuat data siswa binaan...</p></div>';

  google.script.run
    .withSuccessHandler(function(data) {
      monitoringPKLState.data = data || { guru: null, siswa: [], totalSiswa: 0 };
      renderMonitoringPKLGuru();
    })
    .withFailureHandler(function(err) {
      container.innerHTML = '<div class="card" style="background:#fee2e2;padding:18px;">' +
        '<div style="font-weight:800;color:#991b1b;">Gagal memuat data</div>' +
        '<div style="font-size:12.5px;color:#7f1d1d;margin-top:6px;">' + escapeHtml(err.message) + '</div>' +
        '<button class="btn btn-outline btn-sm" onclick="loadMonitoringPKLGuru()" style="margin-top:10px;">' +
...
function renderMonitoringPKLGuru() {
    ...
    html +=
      '<div class="card" style="background:var(--primary-light);border:1px solid var(--primary-border);margin-bottom:16px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">' +
          '<div>' +
            '<div style="font-size:16px;font-weight:800;color:var(--primary-dark);"><i class="fas fa-chalkboard-teacher"></i> ' + escapeHtml(guru.Nama_GTK) + '</div>' +
            '<div style="font-size:12px;color:#065f46;margin-top:2px;">NBM: ' + escapeHtml(guru.NBM) + ' | Tugas: ' + escapeHtml(guru.Tugas || '-') + '</div>' +
          '</div>' +
  ...
  html +=
    '<div class="stat-grid" style="margin-bottom:16px;">' +
      '<div class="stat-card" style="background:linear-gradient(135deg,#10b981,#059669);">' +
        '<div class="stat-label">Siswa Aktif</div><div class="stat-value">' + countAktif + '</div>' +
        '<i class="fas fa-user-check stat-icon"></i></div>' +
      '<div class="stat-card" style="background:linear-gradient(135deg,#94a3b8,#64748b);">' +
        '<div class="stat-label">Selesai</div><div class="stat-value">' + countSelesai + '</div>' +
        '<i class="fas fa-flag-checkered stat-icon"></i></div>' +
      '<div class="stat-card" style="background:linear-gradient(135deg,#f59e0b,#d97706);">' +
        '<div class="stat-label">Jurnal Perlu Approve</div><div class="stat-value">' + totalJurnalPending + '</div>' +
        '<i class="fas fa-clock stat-icon"></i></div>' +
      '<div class="stat-card" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed);">' +
        '<div class="stat-label">Total DUDI</div><div class="stat-value">' + uniqueDudiCount(semuaSiswa) + '</div>' +
        '<i class="fas fa-building stat-icon"></i></div>' +
    '</div>';
  ...
  // List siswa
  if (siswa.length === 0) {
    html += '<div class="card" style="text-align:center;padding:24px;color:var(--text-muted);">' +
      '<i class="fas fa-search" style="font-size:2em;margin-bottom:8px;opacity:0.5;"></i>' +
      '<div style="font-size:12.5px;">Tidak ada siswa yang cocok dengan filter.</div></div>';
    container.innerHTML = html;
    return;
  }
...
function renderMonitoringSiswaCard(s) {
  ...
  var jurnalPendingBadge = s.JurnalPendingSekolah > 0
    ? '<span style="display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:12px;font-size:10.5px;font-weight:800;background:#fef3c7;color:#92400e;border:1px solid #f59e0b;">' +
        '<i class="fas fa-clock"></i> ' + s.JurnalPendingSekolah + ' perlu approve</span>'
    : '';

  var waBtn = s.WA_Pembimbing_DUDI
    ? '<button class="btn btn-wa btn-sm" onclick="monitoringWAkeDudi(\'' + escapeAttr(s.NIS) + '\')" ' +
      'style="min-height:30px;padding:0.25em 0.9em;font-size:11.5px;"><i class="fab fa-whatsapp"></i> WA DUDI</button>'
    : '';
  ...
  return '<div class="card" style="padding:16px;display:flex;flex-direction:column;gap:10px;">' +
    ...
    '<div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:10px 12px;">' +
      '<div style="font-size:10.5px;color:#0369a1;font-weight:800;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:4px;">' +
        '<i class="fas fa-building"></i> DUDI</div>' +
...
function handleMonitoringSearch(val) {
  ...
  window._monitoringSearchTimer = setTimeout(function() {
    var container = document.getElementById('monitoring-pkl-content');
    if (!container) return;
    // Render ulang tanpa kehilangan fokus
...
function monitoringLihatDetail(nis) {
  ...
  var waDudi = s.WA_Pembimbing_DUDI ? '<div style="margin-top:8px;"><i class="fab fa-whatsapp" style="color:#25D366;"></i> <strong>WA DUDI:</strong> ' + escapeHtml(s.WA_Pembimbing_DUDI) + '</div>' : '';
  var alamat = s.Alamat_DUDI ? '<div style="margin-top:4px;"><i class="fas fa-map-marker-alt"></i> <strong>Alamat:</strong> ' + escapeHtml(s.Alamat_DUDI) + '</div>' : '';
  ...
  Swal.fire({
    ...
    html:
      '<div style="text-align:left;font-size:13px;line-height:1.7;">' +
        ...
        '<div style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">NIS: ' + escapeHtml(s.NIS) + ' | Kelas: ' + escapeHtml(s.Nama_Kelas) + '</div>' +
        ...
        '<div style="margin-top:10px;padding:8px 12px;background:#f8fafc;border-radius:8px;font-size:12px;">' +
          '<strong>Periode:</strong> ' + escapeHtml(s.Tanggal_Mulai || '-') + ' s/d ' + escapeHtml(s.Tanggal_Selesai || '-') +
        '</div>' +
      '</div>',
    width: 500,
